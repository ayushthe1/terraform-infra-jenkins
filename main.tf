...
module "jenkins" {
  // setting up ec2 for jenkins
  source                    = "./jenkins"
  ami_id                    = var.ec2_ami_id
  instance_type             = "t2.medium" // t2.micro not sufficient
  ...
  // installing jenkin on ec2 via script
...
module "alb" {
  ...
  is_external               = false
  lb_type                   = "application"
  // security group is same as what we have configured for ec2 instance(jenkins)o
...
module "hosted_zone" {
  source          = "./hosted-zone"
  domain_name     = "jenkins.ayushsharma.co.in"
  aws_lb_dns_name = module.alb.aws_lb_dns_name
  aws_lb_zone_id  = module.alb.aws_lb_zone_id
}


// issue the certificate to enable https request to the loadbalancer
